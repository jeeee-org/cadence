...
- read-onlyではstateに `readonly_hook_status: verified | not-verified | unavailable` と `readonly_hook_deny: observed | not-observed | not-tested` を記録する。Codexの`verified`は`/hooks`でtrusted/enabledを実確認した場合だけ使い、センチネルの存在から推測しない。
...
- 対象リポの実ファイルはread-onlyフローで変更しない。証跡の書込み先は`.cadence/runs/`だけとし、センチネルは上記shell制御操作だけで扱う。
...
- readonly guardはsession cwdの祖先を最初のGitルートまで探索する。対象PJをsession cwdにして開始し、外部targetやouter repoのsentinelがnested Gitリポ内まで効くと仮定しない。
