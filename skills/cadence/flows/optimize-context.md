...
```yaml
purpose: >
  Claude Code プロジェクトのコンテキスト/指示サーフェス（グローバル＋プロジェクト CLAUDE.md、
...
- **触ってよい範囲を確認**：プロジェクト内ファイルのみか、グローバル `~/.claude/`（全PJに影響・版管理外のことが多い＝戻しにくい）も対象に含めるか。
...
- **instruction**:
  - コンテキスト・サーフェスを列挙し、**常時ロード**（毎ターン/セッション開始）と**オンデマンド**を区別：
    - 常時：グローバル `~/.claude/CLAUDE.md`、プロジェクト `CLAUDE.md`、各スキルの `description`/frontmatter、システムリマインダ（memory 残骸含む）、セッション開始時の `PROGRESS.md`。
    - オンデマンド：スキル本体・`references/`・`docs/*`・`docs/checkpoints/`（普段ロードされない）。
  - 規模を測る（`wc -l`／概算トークン）。**常時ロード × 大きい**ものを最優先対象に。
  - 既知の肥大アンチパターンに当たりをつける：global↔project CLAUDE.md の重複／CLAUDE.md に積もったセッション履歴／肥大した PROGRESS（完了項目を checkpoint へ退避すべき）／矛盾する指示／陳腐化した記述／memory 残骸。
...
- **instruction**:
  ...
  - ⚠️ **グローバル `~/.claude/` への変更は別枠で明示**（全PJに影響・版管理外＝戻しにくい）。事前バックアップを提案してから承認を取る。
...
- **edit**: yes
- **instruction**:
  - 承認された変更**だけ**を Edit/Write で適用。スコープ外は触らない（scope creep 禁止）。
...
## convergence
`apply ↔ verify ↔ audit` を `max_cycles`(=3) まで。改善が止まる（削っては別の重複が出る等）なら
**ABORT し、適用済みの変更と未処理候補・（global を触ったなら）バックアップ位置を明記**して人に返す。
...
> 特に **global `~/.claude/` は版管理外で戻しにくい**ため、編集は別承認＋バックアップを徹底する。
