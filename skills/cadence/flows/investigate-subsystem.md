...
mode: read-only
initial: plan
max_cycles: 3          # ハードキャップ。到達で必ず停止（網羅済みなら COMPLETE、未踏ありなら ABORT でマップ済み＋未踏を明示）
```
...
## convergence
`max_cycles`(=3) は**ハードキャップ**：state.md のサイクル数が上限に達したら必ず停止（網羅済みなら
`COMPLETE`、未踏ありなら `ABORT` しマップ済み範囲・未踏範囲・未解決の疑問を正直に出す）。上限前でも、
前サイクルと**未踏リスト・未解決疑問に差分が無い**（新規に読めるパスが出ない）なら停滞として早期 `ABORT`。
