---
name: cadence
description: 構造化レビューループ（plan → audit → supervise → review）で、読み取り専用の監査・調査タスクを宣言的フローとして回す自作ハーネス。フロー定義(flows/*.md)とペルソナ(references/personas/*.md)を読み込み、メインセッションのモデルがステートマシンとして実行する。収束/打ち切りを監視し、決定論ゲート(Bash)と高ステークス判断の quorum 外注を組み込める。SRE 的な「巨大MD＋ライブ状態の欠陥発見→検証」に向く。単純な一問一答には使わない。
---
...
**TAKT には依存しない**（概念の参考のみ）。**メインセッションがエンジン**（モデル非依存）＝フローを読み、各 step をペルソナとして実行し、遷移を判定する。
...
read-only は **SKILL/persona の指示遵守だけに頼らない**。メインセッションは Edit/Write 権限を持つため
