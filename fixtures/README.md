...
同じ理由で、検証ランは**このフィクスチャを作成したセッションとは別のセッション**で回すのが正
（エンジン＝メインセッションが正解を知っていると plan/supervise/fuse の品質評価が汚染される。
audit step はサブエージェントに正解を読ませないことで公平性を担保できるが、完全ではない）。
